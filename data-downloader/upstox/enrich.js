// upstox/enrich.js (was scripts/backfillUpstox.js) — Resumable historical backfill for
...
// at daily granularity, see optionchain/run.js (discovery phase) instead.
//
// Usage: node upstox/enrich.js [SYMBOL] [STRIKES_PER_SIDE] [FROM_DATE] [TO_DATE]
//   SYMBOL           - a symbol, or ALL (every symbol already in
//                       option_chain_history — see optionchain/run.js's discovery phase)
...
//                       Example: node upstox/enrich.js ALL 10 2026-07-01 2026-08-19
//
...
async function backfillExpiry(symbol, underlyingKey, expirySql, strikesPerSide) {
    ...
    if (liquidStrikes) {
        ...
        console.warn(`[upstox] ${symbol} ${expirySql}: no Bhavcopy OI data found for this expiry yet — falling back to ±${strikesPerSide} strikes around expiry-day close. Run optionchain/run.js's discovery phase for this range first for full liquid-strike coverage.`);
    }
...
async function main() {
    ...
    // ALL mode: every symbol already discovered by optionchain/run.js's discovery phase
    ...
    const discoverFrom = fromDate || addDays(todayIst(), -190);
    const [rows] = await pool.query(
    ...
    if (!symbols.length) {
        console.log("[upstox-all] nothing to do — run optionchain/run.js (discovery phase) for a recent range first.");
        await pool.end();
        return;
    }
